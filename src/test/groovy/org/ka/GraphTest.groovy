...
class GraphTest extends Specification {
    ...
    List<Node> minPathDijkstra(Node from, Node to) {
    ...
    def 'find min path from root to a given node with the Dijkstra algorithm'() {
        given:
        Node root = testGraph()
        ...
        expect:
        minPathDijkstra(root, node5)*.value == ['node1', 'node2', 'node8', 'node5' ]
    }

    private Node testGraph() {
